...
def enwik8(path, n_train=int(90e6), n_valid=int(5e6), n_test=int(5e6)):
    ...
    with gzip.open(path) if path.endswith(".gz") else open(path) as file:
        ...
        return (
            torch.from_numpy(trX),
            torch.from_numpy(vaX),
            torch.from_numpy(teX),
        )
...
def go(arg):
    ...
    for i in tqdm.trange(arg.num_batches):
        ...
        source = torch.cat([s[None, :] for s in seqs_source], dim=0).to(
            torch.long
        )
        target = torch.cat([s[None, :] for s in seqs_target], dim=0).to(
            torch.long
        )
        ...
        if i != 0 and (i % arg.test_every == 0 or i == arg.num_batches - 1):
            ...
            upto = (
                data_test.size(0)
                if i == arg.num_batches - 1
                else arg.test_subset
            )
            ...
            with torch.no_grad():
                ...
                for current in range(data_sub.size(0)):
                    ...
                    if (
                        ...
                        lnprobs = output[
                            torch.arange(b, device=d()), -1, target
                        ]
                ...
                tbw.add_scalar(
                    f"transformer/eval-loss", bits_per_byte, i * arg.batch_size
                )
